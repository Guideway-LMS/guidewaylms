 ...
 * @since 2.0
 */
class Model extends JBaseModel implements IGptranslateModel {
	use Exceptions;
	...
	protected function storeActionLog($action, $context, $contextDescription, $link, $itemId) {
		...
		$messageObject->title = 'com_gptranslate';
		$messageObject->extension_name = 'com_gptranslate';
		...
		$actionLogTable->message_language_key = Text::sprintf('COM_GPTRANSLATE_ACTIONLOGS_STRING', $action, $contextDescription);
		$actionLogTable->message = json_encode($messageObject);
		$actionLogTable->log_date = (string)Factory::getDate();
		$actionLogTable->extension = 'com_gptranslate.' . $context;
	...
	public function loadEntity($id) {
		...
		try {
			...
			if (! $sessionData) {
				...
				if (! $table->load ( $id )) {
					throw new GptranslateException ( Text::_ ( 'COM_GPTRANSLATE_ERROR_RECORD_NOT_FOUND' ), 'error' );
				}
			} else {
	...
	public function storeEntity($updateNulls = false) {
		...
		if($this->getComponentParams()->get('actionlogs_integration', 0)) {
			...
			$action = $isNew ? Text::_('COM_GPTRANSLATE_ACTIONLOGS_ADDED') : Text::_('COM_GPTRANSLATE_ACTIONLOGS_UPDATED');
			$contextClass = (new \ReflectionClass($this))->getShortName();
			$context = StringHelper::str_ireplace('model', '', StringHelper::strtolower($contextClass));
			$this->storeActionLog($action, $context, Text::sprintf('COM_GPTRANSLATE_ACTIONLOGS_GENERIC_STORE_ACTION', $table->id, $table->getTableName()), 'index.php?option=com_gptranslate&task=' . $context . '.editEntity&cid[]=' . $table->id, $table->id);
		}
		return $table;
	}
	...
	public function publishEntities($idEntity, $state): bool {
		...
		if (isset ( $idEntity ) && $idEntity) {
			try {
				...
				if (! $table->publish ( $idEntity, $state, $this->user->id )) {
					throw new GptranslateException ( $table->getException (), 'notice' );
				}
			} catch ( GptranslateException $e ) {
				$this->setException ( $e );
				return false;
			} catch ( \Exception $e ) {
				$gptranslateException = new GptranslateException ( $e->getMessage (), 'notice' );
				$this->setException ( $gptranslateException );
				return false;
			}
		}
		...
		// If integration with ActionLogs
		if($this->getComponentParams()->get('actionlogs_integration', 0)) {
			$isPublished = $state ? true : false;
			$action = $isPublished ? Text::_('COM_GPTRANSLATE_ACTIONLOGS_PUBLISHED') : Text::_('COM_GPTRANSLATE_ACTIONLOGS_UNPUBLISHED');
			$contextClass = (new \ReflectionClass($this))->getShortName();
			$context = StringHelper::str_ireplace('model', '', StringHelper::strtolower($contextClass));
			$this->storeActionLog($action, $context, Text::sprintf('COM_GPTRANSLATE_ACTIONLOGS_GENERIC_STORE_ACTION', $idEntity[0], $table->getTableName()), 'index.php?option=com_gptranslate&task=' . $context . '.editEntity&cid[]=' . $table->id, $table->id);
		}
		...
		if (isset ( $idEntity ) && $idEntity) {
			try {
				...
				if (! $table->load ( ( int ) $idEntity )) {
					throw new GptranslateException ( Text::_ ( 'COM_GPTRANSLATE_ERROR_RECORD_NOT_FOUND' ), 'notice' );
				}
			...
			try {
				foreach ( $ids as $id ) {
					if ($table->load ( ( int ) $id )) {
						$table->id = 0;
						$table->name = Text::_ ( 'COM_GPTRANSLATE_COPYOF' ) . $table->name;
						$table->published = 0;
	...
	public function getLists($record = null): array {
		$lists = [];
		// Grid states
		$lists ['published'] = GptranslateHelpersHtml::booleanlist ( 'published', null, $record->published );
	...
	public function getComponentParams(): Registry {
		...
		if (isset($this->app) && $this->app->isClient('site') && $this->option == 'com_gptranslate') {
			$this->componentParams = $this->app->getParams ( 'com_gptranslate' );
		} else {
			$this->componentParams = ComponentHelper::getParams ( 'com_gptranslate' );
		}
	...
	public function __construct($config = array(), ?MVCFactoryInterface $factory = null) {
		...
		Table::addIncludePath ( JPATH_ROOT . '/administrator/components/com_gptranslate/Table' );
