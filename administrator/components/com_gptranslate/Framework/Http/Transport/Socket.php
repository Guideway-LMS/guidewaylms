<?php
namespace JExtstore\Component\Gptranslate\Administrator\Framework\Http\Transport;
/**
...
use JExtstore\Component\Gptranslate\Administrator\Framework\Http\Transport as ITransport;
use JExtstore\Component\Gptranslate\Administrator\Framework\Http\Response;
...
class Socket implements ITransport {
	 ...
	 *  
	 * @since   11.3
	 * @throws  \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Runtime
	 */
	public function __construct() {
		if (!function_exists('fsockopen') || !is_callable('fsockopen')) {
			throw new \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Runtime('Cannot use a socket transport when fsockopen() is not available.', 'error');
		}
	...
	public function request($method, Uri $uri, $data = null, ?array $headers = null, $timeout = 25, $userAgent = null) {
		...
		if (is_resource($connection)) {
			...
			if ($meta['timed_out']) {
				throw new \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Runtime('Server connection timed out.', 'error');
			}
		} else {
			throw new \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Runtime('Not connected to server.', 'error');
		}
	 ...
	 *
	 * @since   11.3
	 * @throws  \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Unexpectedvalue
	 */
	protected function getResponse($content) {
		...
		if (is_numeric($code)) {
			...
			throw new \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Unexpectedvalue('No HTTP response code found.', 'notice');
		}
	...
	protected function connect(Uri $uri, $timeout = null) {
		...
		if (!$connection) {
			throw new \JExtstore\Component\Gptranslate\Administrator\Framework\Exception\Runtime($err, 'error', $errno);
		}
