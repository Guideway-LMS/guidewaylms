...
class Controller extends JBaseController {
	...
	public function editEntity(): bool {
		...
		if (! $record = $model->loadEntity ( $idEntity )) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_EDITING' ) );
			return false;
		}
		...
		if ($record->checked_out && $record->checked_out != $this->user->id) {
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_CHECKEDOUT_RECORD' ), 'notice' );
			return false;
		}
		
		// Access check
		if ($record->id && ! $this->allowEdit ( $this->option )) {
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_ALERT_NOACCESS' ), 'notice' );
			return false;
		}
	...
	public function saveEntity(): bool {
		...
		if (! $result = $model->storeEntity ()) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".editEntity&cid[]=" . $this->app->getInput()->get ( 'id' ), Text::_ ( 'COM_GPTRANSLATE_ERROR_SAVING' ) );
			return false;
		}
		...
		$msg = 'COM_GPTRANSLATE_SUCCESS' . $redirects ['msgsufix'];
	...
	public function cancelEntity(): void {
		...
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_CANCELED_OPERATION' ) );
	}
	...
	public function copyEntity(): bool {
		...
		if (! $model->copyEntity ( $cids )) {
			...
			$modelException = $model->getException ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getExceptionLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_DUPLICATING' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_SUCCESS_DUPLICATING' ) );
	...
	public function deleteEntity(): bool {
		...
		if (! $model->deleteEntity ( $cids )) {
			...
			$modelException = $model->getException ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getExceptionLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_DELETE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_SUCCESS_DELETE' ) );
	...
	public function moveOrder(): bool {
		...
		if (! $model->changeOrder ( $idEntity, $direction )) {
			...
			$modelException = $model->getException ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getExceptionLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_REORDER' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_SUCCESS_REORDER' ) );
	...
	public function publishEntities(): bool {
		...
		if (! $model->publishEntities ( $idEntity, $this->task )) {
			...
			$modelException = $model->getException ( null, false );
			$this->app->enqueueMessage ( $modelException->getMessage (), $modelException->getExceptionLevel () );
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_ERROR_STATE_CHANGE' ) );
			return false;
		}
		
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_SUCCESS_STATE_CHANGE' ) );
	...
	public function checkin(): bool {
		...
		$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->name . ".display", Text::_ ( 'COM_GPTRANSLATE_CHECKEDIN_RECORD' ) );
