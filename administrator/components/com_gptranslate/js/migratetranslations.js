...
//'use strict';
(function($) {
	var MigrateTranslations = function() {
		/**
		...
		var migrationSnippet ='<div id="migraterow" style="display: none;">' +
								'<span class="input-group">' +
									'<span class="input-group-text"><span class="fas fa-globe" aria-hidden="true"></span> ' + COM_GPTRANSLATE_MIGRATE_META_PREVIOUS_DOMAIN + '</span>' +
								...
								'<span class="input-group ms-1">' +
    								'<span class="input-group-text"><span class="fas fa-globe" aria-hidden="true"></span> ' + COM_GPTRANSLATE_MIGRATE_META_NEW_DOMAIN + '</span>' +
    								'<input type="text" class="form-control" id="migratetranslations_newdomain" name="migratetranslations_newdomain" value="">' +
    							'</span>' +
								'<button class="btn btn-primary btn-sm ms-1" id="migrationconfirm">' + COM_GPTRANSLATE_MIGRATE_META_CONFIRM + '</button> ' +
								'<button class="btn btn-primary btn-sm" id="migrationcancel">' + COM_GPTRANSLATE_MIGRATE_META_CANCEL + '</button>' +
							'</div>';
		...
		(function __construct() {
			...
			$('joomla-toolbar-button[task="translations.migrateEntities"]').removeAttr('task').on('click', function(jqEvent){
			...
			$('#migrationconfirm').on('click', function(jqEvent){
				// Validate input
				var currentDomain = $('#migratetranslations_currentdomain');
				var newDomain = $('#migratetranslations_newdomain');
				...
				if(!currentDomainValue) {
					...
					$('#migraterow').append('<span class="validation badge bg-danger">' + COM_GPTRANSLATE_REQUIRED + '</span>');
				...
				if(!fieldValidator(currentDomainValue, "^https?://(.+.)+.{2,4}(/.*)?$")) {
					currentDomain.css('border', '1px solid #F00');
					$('#migraterow span.validation.bg-danger').remove();
					$('#migraterow').append('<span class="validation badge bg-danger">' + COM_GPTRANSLATE_INVALID_URL + '</span>');
					currentDomain.on('click', function(jqEvent){
						$(this).css('border', '1px solid #ccc').next('span.validation').remove();
					});
					return false;
	...
	// On DOM Ready
	$(function() {
		window.GPTranslateMigrateTranslations = new MigrateTranslations();
	});
})(jQuery);