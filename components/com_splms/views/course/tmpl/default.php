...
<div id="splms" class="splms view-splms-course course-details">
    <div class="splms-course">
        ...
        </div> <?php if ($this->item->description) { ?>
        ...
        <?php } ?>
        <?php
        // Apenas exibe o mural se o aluno estiver matriculado ($this->isAuthorised)
        // A view 'announcements' (que estamos a chamar) já tem a sua própria
        // verificação de segurança (Tarefa 1.5.2), mas esta verificação aqui 
        // evita mostrar o título da seção para quem não tem acesso.
        ?>
        <?php if ($this->isAuthorised != '') : ?>
            <div class="splms-course-announcements splms-section">
                <h3 class="splms-title"><?php echo Text::_('COM_SPLMS_ANNOUNCEMENTS'); // TODO: Criar esta string de idioma ?></h3>
                
                <?php
                // Isto carrega a nossa 'view=announcements' que criámos.
                // O Model (Tarefa 1.5.2) irá automaticamente pegar o 'id'
                // do curso a partir da URL principal desta página.
                try {
                    ...
                    Factory::getApplication()->enqueueMessage('Erro ao carregar o mural de avisos: ' . $e->getMessage(), 'error');
                }
                ?>
            </div>
        <?php endif; ?>
