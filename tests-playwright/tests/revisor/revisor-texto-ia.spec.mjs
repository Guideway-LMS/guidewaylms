...
const revisarButton = page.locator('button:has-text("Revisar")');

// Aguarda o botão aparecer
await revisarButton.waitFor({ state: 'visible' });

// Verifica se está habilitado
if (await revisarButton.isEnabled()) {
  await revisarButton.click();
} else {
  throw new Error('Botão "Revisar" está desabilitado.');
}
});
